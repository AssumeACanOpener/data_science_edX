...
def drawPlots(model, wintitle='Figure 1'):
  ...
  colors = [y_colors[i] for i in y_train.values.ravel()]
  ...
  for col in range(num_columns):
    for row in range(num_columns):
      ...
      model.fit(X_train_bag, y_train.values.ravel())
...
#
# .. your code here ..
y = pd.DataFrame(X.wheat_type.map({'kama': 0, 'rosa': 1, 'canadian': 2}))
...
dt = DecisionTreeClassifier(max_depth=9, random_state=2)
